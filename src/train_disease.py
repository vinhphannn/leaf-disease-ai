...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train per-species disease classifier")
    p.add_argument("--data_dir", type=str, default="data")
    p.add_argument("--species", type=str, required=True, help="Target species, e.g., Apple, Tomato")
    p.add_argument("--output_dir", type=str, default="models/disease")
...
def create_disease_loaders(data_dir: Path, species: str, img_size: int, batch_size: int, num_workers: int):
...
def main():
    ...
    out_dir = out_root / args.species
    ensure_dir(out_dir)

    device_name, device = select_device(args.device)
    print(f"Using device: {device_name}")

    train_loader, val_loader, disease_list = create_disease_loaders(data_dir, args.species, args.img_size, args.batch_size, args.num_workers)
    save_class_names(out_dir / "classes.json", disease_list)
    ...
    model = nn.Sequential(
        backbone,
        nn.Linear(in_features, len(disease_list)),
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for imgs, labels in pbar:
            ...
            targets_soft = None
            if use_cutmix:
                imgs, targets_soft = cutmix(imgs, labels, len(disease_list), cutmix_alpha)
            elif use_mixup:
                imgs, targets_soft = mixup(imgs, labels, len(disease_list), mixup_alpha)
    ...
    torch.save({"state_dict": model.state_dict(), "classes": disease_list}, out_dir / "model_final.pt")
    if best_state is not None:
        torch.save({"state_dict": best_state, "classes": disease_list, "best_val_acc": best_acc}, out_dir / "model_best.pt")
        print(f"Saved best disease model for {args.species} acc={best_acc:.4f}")
    ...
    try:
        if val_labels_np.size > 0 and val_preds_np.size > 0:
            save_eval_artifacts(val_labels_np, val_preds_np, disease_list, out_dir)
    except Exception:
        pass
